...
public:
    ...
    vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
        term = start; //for path generation
	    ...
	    //cout<<"level++"<<level<<endl;
            for( auto i: current ) visited.insert(i);
            for(auto currentV: current) {
                string popV = currentV;
                for(int j = 0; j<popV.size(); j++ ){                
                    for( char i = 'a'; i<='z';i++ ){
                        ...
                        if( popV == end ){
                            found = true;
			    //cout<<"true!!!!!!!!!!!!"<<endl;
			    pre[popV].push_back( org );
			}
                        if( popV!=end && (visited.count(popV)==0 && dict.find(popV) != dict.end()) ){ //won't point to same level, including self
                            next.insert(popV);
                            pre[popV].push_back( org );
                            //visited.insert(popV); //  Don't add this line!!! If the end was the bottom of diamond
                        }
                        swap(i, popV[j]);
                    }
                }
            ...
            swap(current, next);
            //VERY IMPORTANT !!!!! Clear!! Otherwise looks like DFS
	    ...
	    //cout<<"found"<<endl;
            ...
            vector<string> tmp;
            path_Gen( pre, end, result, tmp);
            return result;
        }else
        return vector<vector<string> > ();
    }
    
    void path_Gen( unordered_map<string, vector<string> > &pre, string &word,
              vector<vector<string> > &result, vector<string> &tmp){
        if( word == term ){
            vector<string> reverseV;
            reverseV.push_back(word);
            for( auto i = tmp.rbegin(); i!= tmp.rend(); i++)
                reverseV.push_back(*i);
            result.push_back(reverseV);
        }
        //above is termination
        //DFS: pre/ dfs/ post
        //linked nodes are from map
        for(auto i: pre[word]){
            //pre: push the word
            tmp.push_back(word);
            //dfs
            path_Gen(pre, i, result, tmp);
            //post: pop()
            tmp.pop_back();
        }
    }
...
//
    Solution sol;
    vector<vector<string> > n = sol.findLadders( start, end , dict );
    //cout<<"size"<<n.size()<<endl;
    for_each(n.begin(), n.end(), [&](vector<string> vec){
